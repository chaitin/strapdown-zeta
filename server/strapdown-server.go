...
func parseConfig() {
	...
	flag.StringVar(&wikiConfig.extract, "extract", "", "Extract assets so you can modify")
...
func bootstrap() {
	...
	if len(wikiConfig.extract) > 0 {
		// extract and exit
		files := AssetNames()

		for _, name := range files {
			if strings.EqualFold(name, "_static/.md") || strings.EqualFold(name, "_static/version") {
